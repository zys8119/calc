...
import type { Options, RequestResponseType, Store } from "./typings";
...
service.interceptors.request.use(
  (config) => {
      ...
      if (typeof config.encryptKey === "string") {
        ...
        for (const key of config.encryptKey) {
          ...
          if (config.params?.[key])
            config.params[key] = sha256(config.params[key]).toString();
        }
      }
    }
    ...
    if (config.method === "get" && config.params) {
      ...
      if (extra !== "?" && extra !== "&") {
        config.url = (config.url || "") + split + transParams(config.params);
      }
      config.params = {};
    }
...
export const request = <T = any, Config extends AxiosRequestConfig = {}>(
  ...
  if (config.zlbApi && options.zlbAppKey) {
    return zlbReuqest<T, Config>(config, setLoading, options, source.cancel);
  } else {
