...
service.interceptors.request.use(
  (config) => {
    ...
    if (!config.timeout && options.timeout) config.timeout = options.timeout;
    // beforeEach Hook
    ...
    if (config.encryptKey) {
      if (typeof config.encryptKey === "string") {
        if (
          (_a = config.data) === null || _a === void 0
            ? void 0
            : _a[config.encryptKey]
        )
          config.data[config.encryptKey] = (0, sha256_1.default)(
            config.data[config.encryptKey],
          ).toString();
        if (
          (_b = config.params) === null || _b === void 0
            ? void 0
            : _b[config.encryptKey]
        )
          config.params[config.encryptKey] = (0, sha256_1.default)(
            config.params[config.encryptKey],
          ).toString();
      } else {
        for (const key of config.encryptKey) {
          if ((_c = config.data) === null || _c === void 0 ? void 0 : _c[key])
            config.data[key] = (0, sha256_1.default)(
              config.data[key],
            ).toString();
          if ((_d = config.params) === null || _d === void 0 ? void 0 : _d[key])
            config.params[key] = (0, sha256_1.default)(
              config.params[key],
            ).toString();
        }
      }
    ...
    if (config.method === "get" && config.params) {
      const split = (
        (_e = config.url) === null || _e === void 0 ? void 0 : _e.includes("?")
      )
        ? "&"
        : "?";
      ...
      if (extra !== "?" && extra !== "&") {
        config.url =
          (config.url || "") + split + (0, utils_1.transParams)(config.params);
      }
      config.params = {};
    }
    // 添加 base url
    config.url =
      (config.baseURL || config.ignoreGlobalBaseURL ? "" : options.baseURL) +
      (config.url || "");
    return config;
  },
  (error) => {
...
service.interceptors.response.use(
  (response) => {
    ...
    if (!success) {
      ...
      (_a = options.errorHandle) === null || _a === void 0
        ? void 0
        : _a.call(options, messageInfo, res);
      for (const code of codes) {
        if (options.logoutCode.includes(res[code])) {
          (_b = options.logout) === null || _b === void 0
            ? void 0
            : _b.call(options, res);
          break;
        }
      ...
      if (
        (_a =
          messageInfo === null || messageInfo === void 0
            ? void 0
            : messageInfo.includes) === null || _a === void 0
          ? void 0
          : _a.call(messageInfo, "timeout")
      ) {
        messageInfo = "请求超时，请重试";
      }
    }
    (_b = options.errorHandle) === null || _b === void 0
      ? void 0
      : _b.call(
          options,
          messageInfo,
    ...
    const finalPromise = new utils_1.RequestPromise((resolve, reject) => {
      request
        .then((res) => {
          ...
          (_a = options.afterEach) === null || _a === void 0
            ? void 0
            : _a.call(options, config, res.data);
        })
        .catch((err) => {
          reject(err);
        })
